...
" Use cwd (sampled when this script is loaded) as a gutentags_project_root indicator.
if !exists('g:gutentags_cwd_as_project_root')
    let g:gutentags_cwd_as_project_root = 0
endif
let s:gutentags_cwd = getcwd()
...
function! s:get_project_root(path) abort
    ...
    while l:path != l:previous_path
        if g:gutentags_cwd_as_project_root == 1
            if l:path == s:gutentags_cwd
...
function! s:setup_gutentags() abort
    if !g:gutentags_enabled
        " Clear previous gutentags settings for the buffer.
        if exists('b:gutentags_file')
            unlet b:gutentags_root
        endif
        if exists('b:gutentags_file')
    ...
    if exists('b:gutentags_file') && 
            \b:gutentags_cwd_as_project_root == g:gutentags_cwd_as_project_root && 
            \!g:gutentags_debug
        " This buffer already has gutentags support.
        call s:trace("Buffer already with tags file '" . b:gutentags_file . "'")
        return
    endif
    ...
    try
        let b:gutentags_root = s:get_project_root(expand('%:p:h'))
        ...
        return
    endtry
    let b:gutentags_cwd_as_project_root = g:gutentags_cwd_as_project_root
    ...
    if g:gutentags_auto_set_tags
        ...
        call s:trace("Prepended '" . fnameescape(b:gutentags_file) . "' to buffer tags")
    endif
    ...
    if l:found < 0
        call s:trace("Added to known_tagfiles")
        ...
        call s:trace("Already in known_tagfiles")
    endif
endfunction
...
augroup gutentags_detect
    autocmd!
    autocmd BufNewFile,BufEnter    *  call s:setup_gutentags()
...
command! GutentagsToggleEnabled :call gutentags#toggle_enabled()
...
function! gutentags#toggle_enabled()
    let g:gutentags_enabled=!g:gutentags_enabled
    call gutentags#rescan()
    if g:gutentags_enabled
        echom "gutentags: Gutentags is enabled."
    else
        echom "gutentags: Gutentags is disabled."
    endif
endfunction

function! gutentags#is_enabled()
    if g:gutentags_enabled
        echom "gutentags: Gutentags is enabled."
    else
        echom "gutentags: Gutentags is disabled."
...
function! gutentags#toggle_cwd_as_project_root()
    let g:gutentags_cwd_as_project_root=!g:gutentags_cwd_as_project_root
    call gutentags#rescan()
    if g:gutentags_cwd_as_project_root
        echom "gutentags: cwd as project root is enabled."
    else
        echom "gutentags: cwd as project root is disabled."
    endif
endfunction

function! gutentags#is_cwd_as_project_root()
    if g:gutentags_cwd_as_project_root
        echom "gutentags: cwd as project root is enabled."
    else
        echom "gutentags: cwd as project root is disabled."
    endif
    return g:gutentags_cwd_as_project_root
endfunction
